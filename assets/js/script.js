...
var template = {
	...
	header: function(){
		if($('.yv-elementor-header')[0]) {
			...
			menuTl
				...
				.from($menu.find('.g__background'), 0.4, {width:'0%', ease:Power2.easeOut}, 'one')
				.staggerFrom($menu.find('.el-account, .el-menu > ul > li, .el-button'), 0.6, {delay:0.2, opacity:0, y:20, ease:Power2.easeOut}, 0.1, 'one')
				
				.from($menu.find('.part-bottom-bg'), 0.4, {width:'0%', delay:0.5, ease:Power2.easeOut}, 'one')
	...
	animation: function(){
		...
		$section.each(function(index){
		    ...
		    timeLine.pause();
		    timeLine
		    	.fromTo($(this).find('.yv-anim-hero-1'), 0.5, {opacity:0, y:15}, {opacity:1, y:0, ease:Power3.easeOut}, 'one')
		    	.fromTo($(this).find('.yv-anim-hero-2'), 0.4, {opacity:0, y:15}, {delay:0.1, opacity:1, y:0, ease:Power3.easeOut}, 'one')
		...
		$section.each(function(index){
		    ...
		    timeLine.pause();
		    timeLine
		    	.staggerFromTo($(this).find('.yv-anim-adv-1'), 0.5, {opacity:0, y:15}, {opacity:1, y:0, ease:Power3.easeOut}, 0.1, 'one')
		...
		$section.each(function(index){
		    ...
		    timeLine.pause();
		    timeLine
		    	.fromTo($(this).find('.yv-anim-prod-list_1-1'), 0.5, {opacity:0, y:15}, {opacity:1, y:0, ease:Power3.easeOut}, 'one')
		    	.fromTo($(this).find('.yv-anim-prod-list_1-2'), 1.5, {width:0}, {delay:0.1, width:'100%', ease:Power2.easeInOut}, 'one');
		...
		$item.each(function(index){
		    ...
		    timeLine.pause();
		    timeLine
		    	.fromTo($(this), 0.5, {opacity:0, y:15}, {opacity:1, y:0, ease:Power3.easeOut}, 0.1, 'one')
		...
		$section.each(function(index){
		    ...
		    timeLine.pause();
		    timeLine
		    	.fromTo($(this).find('.yv-anim-prod-list_2-1'), 0.5, {opacity:0, y:15}, {opacity:1, y:0, ease:Power3.easeOut}, 'one')
		    	.fromTo($(this).find('.yv-anim-prod-list_2-2'), 1.5, {width:0}, {delay:0.1, width:'100%', ease:Power2.easeInOut}, 'one');
		...
		$section.each(function(index){
		    ...
		    timeLine.pause();
		    timeLine
		    	.staggerFromTo($(this), 0.5, {opacity:0, y:15}, {opacity:1, y:0, ease:Power3.easeOut}, 0.1, 'one')
		...
		$section.each(function(index){
		    ...
		    timeLine
		    	.fromTo($(this), 0.5, {opacity:0, y:15}, {opacity:1, y:0, ease:Power3.easeOut}, 0.1, 'one')
		    	.fromTo($(this).find('.yv-anim-cta-1'), 0.5, {opacity:0, y:15}, {delay:0.1, opacity:1, y:0, ease:Power3.easeOut}, 0.1, 'one')
		    	.fromTo($(this).find('.yv-anim-cta-2'), 0.5, {opacity:0, y:15}, {delay:0.2, opacity:1, y:0, ease:Power3.easeOut}, 0.1, 'one')
		    	.fromTo($(this).find('.yv-anim-cta-3').find('.elementor-button-wrapper'), 0.5, {opacity:0, y:15}, {delay:0.3, opacity:1, y:0, ease:Power3.easeOut}, 0.1, 'one')
...
// CUSTOM CHECKOUT
if ($('body.woocommerce-checkout')[0] && $('.type-custom-checkout')[0] && !$('.woocommerce-order-received')[0]) {
	...
	setTimeout(function(){
		...
		if ($('#shipping_country').attr('type') == 'hidden') {
			$yvShippingPais.val($('#shipping_country_field').find('.woocommerce-input-wrapper strong').text());
		} else {
			$yvShippingPais.replaceWith('<select id="' + $yvShippingPais.attr('id') + '">');
	...
	function checkIfCanOpenThirdStep(updateShipping) {
		...
		if ($yvShippingPais.val() != '' && $yvShippingProvincia.val() != '' && $yvShippingCanton.val() != '' && $yvShippingDistrito.val() != '' && $yvShippingPostal.val() != '' && $yvShippingExacta.val() != '') {
			if (mapClicked == true) {
				if ($('section.type-custom-checkout .step-content-shipping-var-item input').is(':checked')) {
					if (hideLocalPickUp === true || ($('section.type-custom-checkout .step-content-shipping-var-item input:checked').parents('.step-content-shipping-var-item').index() != 0 && $('section.type-custom-checkout .step-content-shipping-var-item').length > 1)) {
	...
	var $yvShippingMethodList = $('#yv_shipping_method');
		yvShippingMethodItemHTML = $('#yv_shipping_method .step-content-shipping-var-item').addClass('yv-new').parent().html();
		...
		jQuery('body').on('updated_checkout', function(){
		    ...
		    if ($shippingMethodItem[0]) {
		    	$shippingMethodItem.each(function(i){
		    		...
		    		$yvShippingMethodList.find('.yv-new').attr('data-shipping-value', $(this).find('input').val());
		    		$yvShippingMethodList.find('.yv-new').find('.step-content-shipping-var-item-text .g__text').text($(this).find('label')[0].childNodes[0].nodeValue);
		    		$yvShippingMethodList.find('.yv-new').find('.step-content-shipping-var-item-text-2 .g__text').text($(this).find('.amount').text());
		    ...
		    $yvShippingMethodList.find('.step-content-shipping-var-item[data-shipping-value="' + 'avfdeliveries-instorepickup0' + '"]').hide();
		    $yvShippingMethodList.find('.step-content-shipping-var-item[data-shipping-value="' + 'avfdeliveries-instorepickup' + '"]').hide();
		    $yvShippingMethodList.find('.step-content-shipping-var-item[data-shipping-value="' + 'local_pickup:16' + '"]').hide();
	...
	$yvShippingMethodList.on('click', '.step-content-shipping-var-item', function(){
		var $shippingMethodItem = $('body').find('#shipping_method li').eq($(this).index()).find('input');
	...
	function bindChangeOnPais() {
		$yvShippingPais.bind('input propertychange', function(){
			...
			if (differentBillingAddress == false) $('#billing_country').val($yvShippingPais.val());

			$('#shipping_country').trigger('change');
			if (differentBillingAddress == false) $('#billing_country').trigger('change');
	...
	function changeTagForCanton() {
		if ($('#shipping_city').parent().find('select')[0]) {
			$yvShippingCanton.replaceWith('<select id="' + $yvShippingCanton.attr('id') + '">');
			$yvShippingCanton = $('#yv_billing_canton');
			...
			$yvShippingCanton.replaceWith('<input id="' + $yvShippingCanton.attr('id') + '">');
			$yvShippingCanton = $('#yv_billing_canton');
	...
	$yvShippingProvincia.bind('input propertychange', function(){
		...
		if (differentBillingAddress == false) $('#billing_state').val($yvShippingProvincia.val());
	...
	function bingChangeOnCanton() {
		$yvShippingCanton.bind('input propertychange', function(){
			...
			if (differentBillingAddress == false) $('#billing_city').val($yvShippingCanton.val());

			$('#shipping_city').trigger('change');
			if (differentBillingAddress == false) $('#billing_city').trigger('change');
	...
	$yvShippingDistrito.bind('input propertychange', function(){
		...
		if (differentBillingAddress == false) $('#billing_address_1').val($yvShippingDistrito.val());
	...
	$yvShippingPostal.bind('input propertychange', function(){
		...
		if (differentBillingAddress == false) $('#billing_postcode').val($yvShippingPostal.val());
	...
	$yvShippingExacta.bind('input propertychange', function(){
		...
		if (differentBillingAddress == false) $('#billing_exact_direction').val($yvShippingExacta.val());
	...
	$shippingOrPick.on('click', function(){
		...
		if( $('#yv_shipping_or_pick_2').is(':checked') ){
			...
			var $shippingMethodItem = $('body').find('#shipping_method li').eq(0).find('input');
	...
	$('.step-content-payment-electronic-facture-label').on('click', function(){
		if ($(this).find('input').is(':checked') ){
			...
			$('#yv_identification_default_info').html(`
				...
				${$yvShippingCanton.find('option:selected')[0] ? $yvShippingCanton.find('option:selected').text() : $yvShippingCanton.val()} / 
	...
	function differentBillingAddressSetupForm() {
		if ($('#billing_country').attr('type') == 'hidden') {
			$yvDifShippingPais.val($('#billing_country_field').find('.woocommerce-input-wrapper strong').text());
		} else {
			$yvDifShippingPais.replaceWith('<select id="' + $yvDifShippingPais.attr('id') + '">');
	...
	function changeTagForDifCanton() {
		if ($('#billing_city').parent().find('select')[0]) {
			$yvDifShippingCanton.replaceWith('<select id="' + $yvDifShippingCanton.attr('id') + '">');
			$yvDifShippingCanton = $('#yv_dif_billing_canton');
			...
			$yvDifShippingCanton.replaceWith('<input id="' + $yvDifShippingCanton.attr('id') + '">');
			$yvDifShippingCanton = $('#yv_dif_billing_canton');
	...
	$('#yv_additional_identification_number').bind('input propertychange', function(){
		...
		if (localPickUp == true) {
			setTimeout(function(){
				if ($('#billing_country').is('select') && !$('#billing_country').val()) {
					$('#billing_country').val($('#billing_country').find('option').eq(1).val());
					$('#billing_country').trigger('change');
				}
				if ($('#shipping_country').is('select') && !$('#shipping_country').val()) {
					$('#shipping_country').val($('#shipping_country').find('option').eq(1).val());
					$('#shipping_country').trigger('change');
				}
			}, 100);
			...
			setTimeout(function(){
				if ($('#billing_state').is('select') && !$('#billing_state').val()) {
					$('#billing_state').val($('#billing_state').find('option').eq(1).val());
					$('#billing_state').trigger('change');
				}
				if ($('#shipping_state').is('select') && !$('#shipping_state').val()) {
					$('#shipping_state').val($('#shipping_state').find('option').eq(1).val());
					$('#shipping_state').trigger('change');
				}
			}, 200);
			...
			setTimeout(function(){
				...
				if ($('#shipping_city').is('select') && !$('#shipping_city').val()) {
					$('#shipping_city').val($('#shipping_city').find('option').eq(1).val());
					$('#shipping_city').trigger('change');
				}
			}, 300);

			setTimeout(function(){
				if ($('#billing_country').val() == '') $('#billing_country').val('Local pickup');
				if ($('#shipping_country').val() == '') $('#shipping_country').val('Local pickup');

				if ($('#billing_state').val() == '') $('#billing_state').val('Local pickup');
				if ($('#shipping_state').val() == '') $('#shipping_state').val('Local pickup');

				if ($('#billing_city').val() == '') $('#billing_city').val('Local pickup');
				if ($('#shipping_city').val() == '') $('#shipping_city').val('Local pickup');

				if ($('#billing_postcode').val() == '') $('#billing_postcode').val('Local pickup');
				if ($('#shipping_postcode').val() == '') $('#shipping_postcode').val('Local pickup');

				if ($('#billing_address_1').val() == '') $('#billing_address_1').val('Local pickup');
				if ($('#shipping_address_1').val() == '') $('#shipping_address_1').val('Local pickup');
			}, 400);
		}
	...
	setInterval(function(){
		...
		if (paymentDestY != 0) {
			$paymentDestination.css('height', $('body').find('.wc_payment_methods').height());

			$('body').find('.wc_payment_methods').css('top', paymentDiffY + 'px');
			$('body').find('.wc_payment_methods').css('left', paymentDiffX + 'px');
			$('body').find('.wc_payment_methods').css('width', $paymentDestination.width());

			$('body').find('.wc_payment_method').each(function(){
				if (!$(this).find('.yv-wc_payment_method-inner')[0]) {
					$(this).prepend(paymentMethodHTML);

					$(this).find('.yv-wc_payment_method-inner').prepend($(this).find('> label'));
					$(this).find('.yv-wc_payment_method-inner').prepend('<div class="yv-wc_payment_method-custom-checkbox"></div>');
					$(this).find('.yv-wc_payment_method-inner').prepend($(this).find('> input'));
				}
			});
		} else {
			$('body').find('.wc_payment_methods').css('top', 0 + 'px');
		}
		// payment end
		...
		if ($('#payment .payment_method_cheque')[0]) {
			if (!$('#payment .payment_method_cheque .yv-payment_box-inner')[0]) {
				$('body').find('.payment_box.payment_method_cheque').append('<div class="yv-payment_box-inner"></div>');
			}
			...
			if ($('#payment_method_cheque').is(':checked')) {
				...
				$('body').find('#alg_checkout_files_upload_form_1').css('top', paymentDiffY + 'px');
				$('body').find('#alg_checkout_files_upload_form_1').css('left', paymentDiffX + 'px');
				$('body').find('#alg_checkout_files_upload_form_1').css('width', $paymentDestination.width());

				$('body').find('.payment_box.payment_method_cheque .yv-payment_box-inner').css('height', $('body').find('#alg_checkout_files_upload_form_1').height());
			} else {
				$('body').find('#alg_checkout_files_upload_form_1').css('top', -10000 + 'px');
				$('body').find('#alg_checkout_files_upload_form_1').removeClass('active');
			}
	...
	setInterval(function(){
		...
		if ($('#shipping_method').find('input:checked').parents('li').find('.woocommerce-Price-amount')[0]) {
			$('#yv_shipping_cost').text($('#shipping_method').find('input:checked').parents('li').find('.woocommerce-Price-amount').text());
		} else {
			$('#yv_shipping_cost').text(0);
		}
		if ($('.order-total .includes_tax .woocommerce-Price-amount')[0]) {
			$('#yv_taxes').text($('.order-total .includes_tax .woocommerce-Price-amount').text());
		} else {
			$('#yv_taxes').text(0);
		}
		if ($('.cart-discount')[0]) {
			$('#yv_discount').text($('.cart-discount .woocommerce-Price-amount').text());
		} else {
			$('#yv_discount').text(0);
		}
...
// CUSTOM FIXES
setInterval(function(){
	$('.elementor-menu-cart__product-image.product-thumbnail').height($('.elementor-menu-cart__product-image.product-thumbnail').width());

	if ($('#yv_billing_pais').val() == 'CR' || $('#yv_billing_pais').val() == 'Costa Rica') {
	...
	if ($('#yv_billing_pais').val() == 'MX' || $('#yv_billing_pais').val() == 'México' || $('#yv_billing_pais').val() == 'Mexico') {
		var text = $('#yv_billing_provincia').parents('.g__form-input').find('.g__form-input-label').data('mexico-text');
		$('#yv_billing_provincia').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_billing_canton').parents('.g__form-input').find('.g__form-input-label').data('mexico-text');
		$('#yv_billing_canton').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_billing_distrito').parents('.g__form-input').find('.g__form-input-label').data('mexico-text');
		$('#yv_billing_distrito').parents('.g__form-input').find('.g__form-input-label b').text(text);

		return;
	} else {
		var text = $('#yv_billing_provincia').parents('.g__form-input').find('.g__form-input-label').data('text');
		$('#yv_billing_provincia').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_billing_canton').parents('.g__form-input').find('.g__form-input-label').data('text');
		$('#yv_billing_canton').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_billing_distrito').parents('.g__form-input').find('.g__form-input-label').data('text');
		$('#yv_billing_distrito').parents('.g__form-input').find('.g__form-input-label b').text(text);
	}

	if ($('#yv_billing_pais').val() == 'CR' || $('#yv_billing_pais').val() == 'Costa Rica') {
		var text = $('#yv_billing_canton').parents('.g__form-input').find('.g__form-input-label').data('costa-rica-text');
		$('#yv_billing_canton').parents('.g__form-input').find('.g__form-input-label b').text(text);
...
// DIFF FACTURA
setInterval(function(){
	if ($('#yv_dif_billing_pais').val() == 'CR' || $('#yv_dif_billing_pais').val() == 'Costa Rica') {
	...
	if ($('#yv_dif_billing_pais').val() == 'MX' || $('#yv_dif_billing_pais').val() == 'México' || $('#yv_dif_billing_pais').val() == 'Mexico') {
		var text = $('#yv_dif_billing_provincia').parents('.g__form-input').find('.g__form-input-label').data('mexico-text');
		$('#yv_dif_billing_provincia').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_dif_billing_canton').parents('.g__form-input').find('.g__form-input-label').data('mexico-text');
		$('#yv_dif_billing_canton').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_dif_billing_distrito').parents('.g__form-input').find('.g__form-input-label').data('mexico-text');
		$('#yv_dif_billing_distrito').parents('.g__form-input').find('.g__form-input-label b').text(text);

		return;
	} else {
		var text = $('#yv_dif_billing_provincia').parents('.g__form-input').find('.g__form-input-label').data('text');
		$('#yv_dif_billing_provincia').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_dif_billing_canton').parents('.g__form-input').find('.g__form-input-label').data('text');
		$('#yv_dif_billing_canton').parents('.g__form-input').find('.g__form-input-label b').text(text);

		var text = $('#yv_dif_billing_distrito').parents('.g__form-input').find('.g__form-input-label').data('text');
		$('#yv_dif_billing_distrito').parents('.g__form-input').find('.g__form-input-label b').text(text);
	}

	if ($('#yv_dif_billing_pais').val() == 'CR' || $('#yv_dif_billing_pais').val() == 'Costa Rica') {
		var text = $('#yv_dif_billing_canton').parents('.g__form-input').find('.g__form-input-label').data('costa-rica-text');
		$('#yv_dif_billing_canton').parents('.g__form-input').find('.g__form-input-label b').text(text);
